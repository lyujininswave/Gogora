...
public class MydataSystemHandleAdapter extends ElSystemHandleAdapter {

 
	public MydataSystemHandleAdapter(Map<String, Object> adapterInfoMap) {
		super(adapterInfoMap);
	}

	@Override
	public void preHandle(HttpServletRequest request, String inputData) throws UserException {
		AppLog.debug("[MydataSystemHandleAdapter-preHandle]...");
		...
		try{
			...
			new MydataAuthProcess().checkAuth(request, svcId, inputData);			
	...
	public void postHandle(ElHeader elHeader, UserHeader userHeader, Object[] serviceParams, Model resultModel) {
		AppLog.debug("[MydataSystemHandleAdapter-postHandle]...");
		...
		if( userHeader != null && userHeader instanceof MydataUserHeader ) {
			MydataUserHeader siteUserHeader = (MydataUserHeader)userHeader;
